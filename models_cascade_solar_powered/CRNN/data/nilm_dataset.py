...
class NILMDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        y_1 = self.padding_seqs(self.y[0][start_index:end_index])
        y_2 = self.padding_seqs(self.y[1][start_index:end_index])
        y_1 = y_1[-self.pred_len:]
        y_2 = y_2[-self.pred_len:]
        return torch.from_numpy(x).float(), [torch.from_numpy(y_1).float(),torch.from_numpy(y_2).float()]
