...
model_name = 'CRNN'
experiment_name = 'VERIFICA'
SOLAR = False

data = 'UKDALE'

appliance_names = ['solar','kettle', 'microwave', 'fridge', 'washing machine', 'dish washer']

seq_len = 600
pred_len = 600
window_stride = 480

seeds = [0,1,2,3,4,5]  # ,4,5
...
if __name__ == '__main__':
    ...
    for seed in seeds:
        ...
        if SOLAR:
            per_app_results_csv = os.path.join(path_csv, '50_per_app_results_SOLAR.csv')
            avg_results_csv = os.path.join(path_csv, 'avg_results_SOLAR.csv')
        else:
            per_app_results_csv = os.path.join(path_csv, '50_per_app_results_NILM.csv')
            avg_results_csv = os.path.join(path_csv, 'avg_results_NILM.csv')
        df_per_app_results = pd.read_csv(per_app_results_csv)
        df_avg_results = pd.read_csv(avg_results_csv)

        for appliance in appliance_names:     # appliance_names
